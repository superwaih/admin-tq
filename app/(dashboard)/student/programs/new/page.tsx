...
function ProgramCard({ prog, isSelected, onAdd, onRemove, onWishlist, wishlisted }: {
  prog: Program;
  isSelected: boolean;
  onAdd: () => void;
  onRemove: () => void;
  ...
  return (
    <div className={`bg-white dark:bg-[#161a27] rounded-2xl border transition-all shadow-sm flex flex-col ${isSelected ? 'border-blue-300 dark:border-blue-500/50 ring-1 ring-blue-200 dark:ring-blue-500/20' : 'border-gray-100 dark:border-white/6 hover:border-blue-200 dark:hover:border-blue-500/30 hover:shadow-md'}`}>
      <div className="p-4 flex-1">
        {/* Top row */}
        <div className="flex items-start justify-between gap-2 mb-3">
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${matchColor(prog.match)}`}>{prog.match}% Match</span>
          <button onClick={onWishlist} className="p-1 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold text-slate-800 dark:text-white leading-tight truncate">{prog.university}</p>
        ...
        {/* Program name */}
        <p className="text-[11px] font-semibold text-slate-700 dark:text-slate-200 mb-3 leading-snug">{prog.program}</p>
      ...
      <div className="max-w-[1380px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="flex-1 min-w-0">
            <Link href="/student/programs" className="inline-flex items-center gap-1.5 text-sm text-slate-500 dark:text-[#8e92ad] hover:text-blue-600 transition-colors mb-2">
