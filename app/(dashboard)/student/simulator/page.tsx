...
import { RotateCcw, Target, Award, Zap, TrendingUp, Info, ChevronRight, BarChart3 } from 'lucide-react';
...
export default function SimulatorPage() {
  ...
  function resetGrades() {
    ORIGINALS.forEach((v, i) => updateGrade?.(i, v));
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Drag any slider — all 5 program probabilities update instantly.
            </p>
          </div>
          <button
            onClick={resetGrades}
            className="self-start sm:self-auto flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/10 shadow-sm transition-all"
          >
                ...
                return (
                  <div key={meta.code} className="px-6 py-5 group">
                    ...
                    <div className="relative h-5 flex items-center">
                      ...
                      {/* Thumb overlay */}
                      <div
                        className="absolute w-5 h-5 rounded-full border-2 border-white dark:border-[#161a27] shadow-md pointer-events-none transition-all duration-150"
                        style={{ left: `calc(${pct}% - 10px)`, backgroundColor: color, boxShadow: `0 0 0 3px ${color}30` }}
                      />
                      {/* Invisible native input */}
                      <input
                        type="range" min={50} max={100} value={value}
                    ...
                    <div key={prob.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50/60 dark:hover:bg-white/3 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className={cn('text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0', pc.badge)}>
                            {pc.label}
                          </span>
                        </div>
                        ...
                        <div className="mt-2.5 h-1.5 bg-gray-100 dark:bg-white/8 rounded-full overflow-hidden">
                          <div className="h-full rounded-full transition-all duration-500"
                            style={{ width: `${prob.probability}%`, backgroundColor: pc.bar }} />
                        </div>
                      </div>
                    </div>
                  );
                  ...
                  <div key={f.label}>
                    ...
                    <div className="h-2.5 bg-gray-100 dark:bg-white/8 rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-700"
                        style={{ width: `${f.impact * 4.5}%`, backgroundColor: f.color }} />
                    </div>
                  </div>
                ))}
              </div>
