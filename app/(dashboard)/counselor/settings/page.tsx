...
import {
  Mail, Phone, Hash, CalendarDays, FileText, Edit2,
  Bell, Smartphone, BookOpen, Users, BarChart3,
  Globe, Clock, Save,
  Shield, Key, Monitor, History, ChevronRight,
  Download, Lock, Trash2, ShieldCheck,
...
function Toggle({ on, onChange }: { on: boolean; onChange: () => void }) {
  return (
    <button
      onClick={onChange}
      className={`relative w-10 h-6 rounded-full transition-colors shrink-0 ${on ? 'bg-cyan-600' : 'bg-gray-200 dark:bg-white/15'}`}
    >
...
function ActionRow({ icon, label, sublabel, extra, danger = false }: {
  icon: React.ReactNode; label: string; sublabel?: string; extra?: React.ReactNode; danger?: boolean;
}) {
  return (
    <button className="w-full flex items-center justify-between py-3 border-b border-gray-50 dark:border-white/[0.04] last:border-0 hover:bg-gray-50 dark:hover:bg-white/[0.02] -mx-1 px-1 rounded-xl transition-colors group">
...
function SelectField({ label, sublabel, value, options }: { label: string; sublabel?: string; value: string; options?: string[] }) {
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-gray-50 dark:border-white/[0.04] last:border-0">
      ...
      <select className="text-[11px] font-semibold text-slate-700 dark:text-[#c8ccdf] bg-gray-50 dark:bg-white/[0.04] border border-gray-100 dark:border-white/8 rounded-lg px-2 py-1 outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-400/50 transition-all cursor-pointer">
        <option>{value}</option>
...
export default function CounselorSettingsPage() {
  ...
  const [profile, setProfile] = useState({
    ...
    email: 'sarah.johnson@admitiq.com',
    phone: '+1 (555) 123-4567',
    id:    'CSL-2020-0412',
    since: 'August 2020',
    bio:   'Experienced college counselor dedicated to helping students achieve their academic goals with personalized guidance and proven strategies.',
  });
  const [editProfile, setEditProfile] = useState({ ...profile });

  function saveProfile() { setProfile({ ...editProfile }); setEditing(false); }

  const tabSections: Record<Tab, string[]> = {
    All:           ['profile', 'notifications', 'preferences', 'security', 'data', 'integrations', 'billing'],
    ...
    <div className="min-h-screen bg-[#F8F9FB] dark:bg-[#0f1117]">
      <div className="max-w-[1200px] mx-auto space-y-5">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-[#8e92ad] mt-1">Manage your account preferences and counselor settings.</p>
        </div>
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none flex-wrap">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all ${
              activeTab === 'All'
                ? 'bg-cyan-600 text-white shadow-sm'
                : 'bg-white dark:bg-[#161a27] text-cyan-600 dark:text-cyan-400 border border-cyan-200 dark:border-cyan-500/30 hover:bg-cyan-50 dark:hover:bg-cyan-500/5'
            }`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              ...
              <div className="flex items-start justify-between mb-5">
                <div className="flex items-center gap-3">
                  <div className="relative shrink-0">
                    <div className="w-14 h-14 rounded-2xl bg-cyan-600 flex items-center justify-center text-white font-bold text-lg">SJ</div>
              ...
              {editing ? (
                  ...
                  {([
                    ...
                    <div key={f.key}>
                      ...
                      <input
                        ...
                        className="w-full h-9 px-3 text-xs bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-white/6 rounded-xl text-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-400/50 transition-all"
                      />
                    </div>
                  ))}
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-3 py-2 text-xs bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-white/6 rounded-xl text-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-400/50 transition-all resize-none"
                    />
                  </div>
                  <button
                    onClick={saveProfile}
                    className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl text-xs font-bold transition-colors"
                  >
                ...
                <div className="space-y-3">
                  {[
                    ...
                    { icon: <Hash size={13} />,        label: 'COUNSELOR ID',     value: profile.id },
                    { icon: <CalendarDays size={13} />, label: 'COUNSELOR SINCE', value: profile.since },
            ...
            <SectionCard title="Notification Preferences" subtitle="Choose how you want to be notified.">
              <div className="space-y-1">
                {([
                  ...
                  { key: 'newRequests',      icon: <Users size={13} />,        label: 'New Student Requests',  sub: 'Get notified about new requests' },
                  { key: 'essayUpdates',     icon: <BookOpen size={13} />,     label: 'Essay Review Updates',  sub: 'Updates on essay reviews' },
                  { key: 'sessionReminders', icon: <Clock size={13} />,        label: 'Session Reminders',     sub: 'Reminders for scheduled sessions' },
          ...
          {visible('preferences') && (
            <SectionCard title="Appearance & Preferences" subtitle="Customize your experience.">
              <div className="space-y-0">
                ...
                <SelectField label="Timezone"    sublabel="Your current timezone"        value="(GMT-5) Eastern Time" options={['(GMT-8) Pacific Time', '(GMT-6) Central Time']} />
        ...
        {/* ── Row 2: Security · Data & Privacy · Integrations / Billing ── */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          ...
          {/* Data & Privacy */}
          {visible('data') && (
            <SectionCard title="Data & Privacy" subtitle="Manage your data and privacy settings.">
          ...
          {visible('integrations') && (
            <SectionCard title="Integrations" subtitle="Connect your apps and services.">
              {[
                { name: 'Google Calendar', sub: 'Sync session schedules', connected: true,  color: 'bg-blue-50 dark:bg-blue-500/10',    tc: 'text-blue-600' },
                { name: 'Zoom',            sub: 'Host video sessions',     connected: true,  color: 'bg-blue-50 dark:bg-blue-500/10',    tc: 'text-blue-600' },
                { name: 'Microsoft Teams', sub: 'Team communication',      connected: false, color: 'bg-violet-50 dark:bg-violet-500/10', tc: 'text-violet-600' },
                { name: 'Slack',           sub: 'Student notifications',   connected: false, color: 'bg-pink-50 dark:bg-pink-500/10',    tc: 'text-pink-600' },
              ].map(int => (
                <div key={int.name} className="flex items-center justify-between py-3 border-b border-gray-50 dark:border-white/[0.04] last:border-0">
                  ...
                  <button className={`text-[10px] font-bold px-2.5 py-1 rounded-lg transition-colors ${
                    int.connected
                      ...
                      : 'bg-cyan-50 dark:bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 hover:bg-cyan-100'
                  }`}>
          ...
          {visible('billing') && (
            <SectionCard title="Account & Subscription" subtitle="Manage your account and subscription details.">
              <div className="flex items-center justify-between p-3.5 bg-cyan-50 dark:bg-cyan-500/10 rounded-xl border border-cyan-100 dark:border-cyan-500/20 mb-4">
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-lg bg-cyan-600 flex items-center justify-center shrink-0">
                    <Star size={14} className="text-white fill-white" />
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-cyan-500 dark:text-cyan-400 uppercase tracking-wider">Current Plan</p>
                    <p className="text-xs font-bold text-cyan-800 dark:text-cyan-200">Premium Plan</p>
                  </div>
                </div>
                <button className="px-3 py-1.5 bg-cyan-600 hover:bg-cyan-700 text-white rounded-lg text-[10px] font-bold transition-colors">
                  Change Plan
                </button>
              </div>
        ...
        {(activeTab === 'Profile' || activeTab === 'Notifications' || activeTab === 'Preferences' || activeTab === 'All') && (
          <div className="flex items-center justify-end gap-3 pt-1 pb-4">
            ...
            <button className="flex items-center gap-1.5 px-5 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-bold transition-colors shadow-sm shadow-cyan-200 dark:shadow-cyan-900/20">
