...
type SessionType = 'Essay Review' | 'Interview Prep' | 'Consultation' | 'Program Strategy' | 'Other';

interface BookedSession {
  id: number;
  initials: string;
  mentor: string;
  title: string;          // counsellor short title
  type: SessionType;
...
const TYPE_COLORS: Record<SessionType, { bg: string; border: string; dot: string; text: string }> = {
  ...
  'Program Strategy': { bg: 'bg-amber-50 dark:bg-amber-500/20',   border: 'border-amber-300 dark:border-amber-500/40',   dot: 'bg-amber-500',    text: 'text-amber-700 dark:text-amber-300'   },
...
const AVATAR_COLORS: Record<string, string> = {
  SC: 'bg-blue-500', MR: 'bg-emerald-500', AP: 'bg-amber-500',
  JW: 'bg-violet-500', PD: 'bg-rose-500', RK: 'bg-cyan-500',
};

// ── Booked-session data (student's own bookings) ───────────────────────────────
const SESSIONS: BookedSession[] = [
  { id: 1, initials: 'SC', mentor: 'Dr. Sarah Chen',   title: 'University Admissions Specialist', type: 'Essay Review',     startHour: 16,   endHour: 17,   dayIndex: 0, venue: 'Zoom',       format: 'Virtual' },
  { id: 2, initials: 'SC', mentor: 'Dr. Sarah Chen',   title: 'University Admissions Specialist', type: 'Program Strategy', startHour: 15,   endHour: 16,   dayIndex: 1, venue: 'Zoom',       format: 'Virtual' },
  { id: 3, initials: 'MR', mentor: 'Michael Roberts',  title: 'Ivy League Essay Coach',           type: 'Consultation',     startHour: 14,   endHour: 14.75, dayIndex: 2, venue: 'Teams',      format: 'Virtual' },
  { id: 4, initials: 'AP', mentor: 'Aisha Patel',      title: 'STEM Admissions Advisor',          type: 'Consultation',     startHour: 13,   endHour: 13.75, dayIndex: 3, venue: 'Video call', format: 'Virtual' },
  { id: 5, initials: 'JW', mentor: 'James Wong',       title: 'Medical School Mentor',            type: 'Interview Prep',   startHour: 19.5, endHour: 20.5, dayIndex: 3, venue: 'Teams',      format: 'Virtual' },
  { id: 6, initials: 'PD', mentor: 'Priya Desai',      title: 'Scholarship Strategist',           type: 'Program Strategy', startHour: 10,   endHour: 11,   dayIndex: 4, venue: 'UofT Campus', format: 'In-person' },
];
...
const VALID_TYPES: SessionType[] = ['Essay Review', 'Interview Prep', 'Consultation', 'Program Strategy'];

// Booking entry points use varied type labels (the book page uses consultation
// labels like "Mock Interview"/"MMI Prep"; the inline dialog uses the session
// medium like "Video Session"). Map them onto a calendar category so paid
// sessions keep readable colours instead of all falling back to "Other".
const TYPE_ALIASES: Record<string, SessionType> = {
  'Mock Interview':  'Interview Prep',
  'MMI Prep':        'Interview Prep',
  'CASPer Coaching': 'Interview Prep',
  'Video Session':   'Consultation',
  'Chat Session':    'Consultation',
  'Phone Session':   'Consultation',
};

function normalizeType(raw: string): SessionType {
  if (VALID_TYPES.includes(raw as SessionType)) return raw as SessionType;
  return TYPE_ALIASES[raw] ?? 'Other';
}
...
function requestToSession(req: SessionPaymentRequest, idx: number): BookedSession | null {
  ...
  const type: SessionType = normalizeType(req.type);
  return {
    id: 1000 + idx,
    initials: req.avatar,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-5 pt-5 pb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 dark:text-[#8e92ad]">All your scheduled counselling and admissions sessions.</p>
          </div>
        </div>
