      ...
      <DialogContent className="max-w-2xl w-full p-0 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#161a27] gap-0 flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="px-6 py-4 border-t border-gray-100 dark:border-white/6 flex items-center justify-between gap-3 bg-white dark:bg-[#161a27]">
          ...
          {c.availability === 'booked' ? (
            ...
            <Link href={`/university-college-student-route/counselors/book?id=${c.id}`}
              onClick={onClose}
...
function BookingDialog({
  ...
  // The student does not book/pay directly: forward the session as a pending
  // payment request to the parent/guardian dashboard.
  const handleConfirm = () => {
    const day = days.find(d => d.value === selectedDay);
    const sd = selectedDay ? new Date(selectedDay) : new Date();
    const dateISO = `${sd.getFullYear()}-${String(sd.getMonth() + 1).padStart(2, '0')}-${String(sd.getDate()).padStart(2, '0')}`;
    const studentName = user
      ? `${user.first_name} ${user.last_name}`.trim()
      : 'Your student';
    addRequest({
      counselorId:    String(c.id),
      counselorName:  c.name,
      counselorTitle: c.title,
      avatar:         c.avatar,
      gradient:       c.avatarGradient,
      type:           sessionType ? `${sessionType} Session` : 'Consultation',
      dateLabel:      day ? `${day.label}, ${day.sub}` : '',
      dateISO,
      time:           selectedTime,
            ...
            <div className="flex flex-col items-center justify-center gap-4 py-12 px-8 text-center">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">
                  {c.price === 0 ? 'Request Sent for Approval' : 'Payment Request Sent'}
                </h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  Your {sessionType} session with <strong className="text-slate-700 dark:text-slate-300">{c.name}</strong> has been sent to your parent / guardian for {c.price === 0 ? 'approval' : 'payment'}.
                </p>
              </div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500">
                {c.price === 0
                  ? 'It will appear on your booking calendar once a parent approves it.'
                  : 'It will appear on your booking calendar once a parent completes the payment.'}
              </p>
        ...
        {!confirmed && (
          <div className="shrink-0 px-5 py-4 border-t border-gray-100 dark:border-white/6 flex items-center gap-3 bg-white dark:bg-[#161a27]">
            ...
            <Button
              ...
              onClick={() => { if (step < 3) setStep(s => s + 1); else handleConfirm(); }}
            ...
            >
              {step === 3 ? (
                <><Check size={14} /> Send to Parent{c.price === 0 ? '' : ` — $${c.price}`}</>
              ) : (
